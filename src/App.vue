      ...
      <v-container>
        ...
        >
          <v-flex mb-4>
            ...
            <p class="subheading font-weight-regular">
              Децентрализованный сбор средств с использованием Etherium
            </p>
          </v-flex>
        </v-layout>
        ...
        <v-layout row justify-center>
          <v-dialog v-model="startProjectDialog" max-width="600px" persistent>
            <v-btn slot="activator" color="primary" dark>Новый проект</v-btn>
            <v-card>
              <v-card-title>
                <span class="headline font-weight-bold mt-2 ml-4">Воплоти свой проект в жизнь</span>
              </v-card-title>
              ...
              >
                <v-card-title primary-title>
                  <div>
                    ...
                    <br/><br/>
                    <small>Сбор заканчивается: <b>{{ new Date(project.deadline * 1000) }}</b></small>
                    <br/><br/>
                    <small>Цель <b>{{ project.goalAmount / 10**18 }} ETH </b></small>
                    <small v-if="project.currentState == 1">не был выполнен в срок</small>
                    <small v-if="project.currentState == 2">цель достигнута</small>
                  </div>
                </v-card-title>
                <v-flex
...
export default {
  ...
  data() {
    return {
      ...
      account: null,
      stateMap: [
        { color: 'primary', text: 'Активен' },
        { color: 'warning', text: 'Просрочен' },
        { color: 'success', text: 'Завершен' },
      ],
      projectData: [],
      newProject: { isLoading: false },
    };
